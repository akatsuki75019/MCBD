class JointTableCartBooksController < ApplicationController
  ...
  def create
    ...
    when JointTableCartBook #dans les cas où l'ajout au panier se fait depuis le show_book
      redirect_to book_path(book) , notice: "Le livre a été ajouté au panier avec succès"
    when "Quantité mise à jour dans le panier"
      redirect_to book_path(book) , notice: "La quantité a été mise à jour dans le panier"
    when "La quantité demandée n'est pas disponible"
      redirect_to book_path(book) , notice: "La quantité demandée n'est pas disponible"
    when "Le livre est en rupture de stock"
      redirect_to book_path(book) , notice: "Le livre est actuellement en rupture de stock"
    else
      redirect_to book_path(book) , notice: "Erreur lors de l'ajout au panier"
    end
  end
